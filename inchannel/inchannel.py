...
LOG: Final = logging.getLogger("simsonfanatiker.inchannel")
...
class inchannel(commands.Cog):
    ...
    async def inchannel(self, ctx: commands.GuildContext):
        """
        Configure or view settings for automated voice-based permissions.
        """

    @inchannel.command(name="unset", require_var_positional=True)
    @commands.guild_only()
    @commands.admin_or_permissions(manage_guild=True)
    async def _unset(
        self,
    ...
    @inchannel.group(name="set", invoke_without_command=True, ignore_extra=False)
    @commands.guild_only()
    @commands.admin_or_permissions(manage_guild=True)
    async def _set(
        self,
    ...
    async def show(
        ...
        *,
        ...
        See `[p]help inchannel set` for explanations of the various settings.
        """
        guild = ctx.guild
